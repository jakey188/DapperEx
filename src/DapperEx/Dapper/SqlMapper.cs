...
#pragma warning restore 618
                ...
                try
                {
                    ...
                    using (var cmd = command.SetupCommand(cnn, null))
                    {
                        ...
                        foreach (var obj in multiExec)
                        {
                            ...
                            total += cmd.ExecuteNonQueryInterceptor();
                        }
                    }
                    command.OnCompleted();
                }
        ...
        private static IDataReader ExecuteReaderWithFlagsFallback(IDbCommand cmd, bool wasClosed, CommandBehavior behavior)
        {
            try
            {
                return cmd.ExecuteReaderInterceptor(GetBehavior(wasClosed, behavior));
            }
...
#pragma warning restore 618
        ...
        private static int ExecuteCommand(IDbConnection cnn, ref CommandDefinition command, Action<IDbCommand, object> paramReader)
        {
            ...
            try
            {
                ...
                if (wasClosed) cnn.Open();
                int result = cmd.ExecuteNonQueryInterceptor();
                command.OnCompleted();
                return result;
            }
            finally
        ...
        private static T ExecuteScalarImpl<T>(IDbConnection cnn, ref CommandDefinition command)
        {
            ...
            try
            {
                cmd = command.SetupCommand(cnn, paramReader);
                if (wasClosed) cnn.Open();
                result = cmd.ExecuteScalarInterceptor();
                command.OnCompleted();
            }
            finally
            {
