...
namespace Dapper
{
    public static partial class SqlMapper
    {
        ...
        {
            var task = cmd.ExecuteReaderAsyncInterceptor(GetBehavior(wasClosed, behavior), cancellationToken);
            ...
            { // we can retry; this time it will have different flags
                return cmd.ExecuteReaderAsyncInterceptor(GetBehavior(wasClosed, behavior), cancellationToken);
            }
            return task;
        }
                ...
                if ((command.Flags & CommandFlags.Pipelined) != 0)
                {
                    ...
                    try
                    {
                        foreach (var obj in multiExec)
                        {
                            ...
                            var task = cmd.ExecuteNonQueryAsyncInterceptor(command.CancellationToken);
                ...
                else
                {
                    using (var cmd = (DbCommand)command.SetupCommand(cnn, null))
                    {
                        foreach (var obj in multiExec)
                        {
                            ...
                            info.ParamReader(cmd, obj);
                            total += await cmd.ExecuteNonQueryAsyncInterceptor(command.CancellationToken).ConfigureAwait(false);
                        }
                    }
                }
        ...
        private static async Task<int> ExecuteImplAsync(IDbConnection cnn, CommandDefinition command, object param)
        {
            ...
            using (var cmd = (DbCommand)command.SetupCommand(cnn, info.ParamReader))
            {
                ...
                {
                    if (wasClosed) await ((DbConnection)cnn).OpenAsync(command.CancellationToken).ConfigureAwait(false);
                    var result = await cmd.ExecuteNonQueryAsyncInterceptor(command.CancellationToken).ConfigureAwait(false);
                    command.OnCompleted();
                    return result;
                }
                finally
        ...
        private static async Task<T> ExecuteScalarImplAsync<T>(IDbConnection cnn, CommandDefinition command)
        {
            ...
            try
            {
                cmd = (DbCommand)command.SetupCommand(cnn, paramReader);
                if (wasClosed) await ((DbConnection)cnn).OpenAsync(command.CancellationToken).ConfigureAwait(false);
                result = await cmd.ExecuteScalarAsyncInterceptor(command.CancellationToken).ConfigureAwait(false);
                command.OnCompleted();
            }
            finally
            {
